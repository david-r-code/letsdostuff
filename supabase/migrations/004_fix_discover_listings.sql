-- Migration 004: Rewrite discover_listings with a more compatible implementation.
-- The original used ARRAY(SELECT unnest(...) INTERSECT SELECT unnest(...)) which can
-- throw 500s depending on PostgREST/PostGIS version. Replaced with a correlated
-- subquery COUNT approach and explicit SRID on the search point.
...
BEGIN
  ...
  SELECT
    ...
    l.created_at,
    ROUND(
      (ST_Distance(l.location_point, v_point) / 1000.0)::numeric,
      1
...
-- Grant execute to authenticated users and anon (needed for unauthenticated browse later)
